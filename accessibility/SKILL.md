...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/accessibility` and restart Codex after major changes.
...
- Fallback prompt: "Use the Accessibility (a11y) skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `accessibility` outside its documented task boundary.
...
Before claiming the `accessibility` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/core-web-vitals` and restart Codex after major changes.
...
- Fallback prompt: "Use the Core Web Vitals optimization skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `core-web-vitals` outside its documented task boundary.
...
Before claiming the `core-web-vitals` workflow succeeded:

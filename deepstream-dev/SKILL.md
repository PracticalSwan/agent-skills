...
- Fallback prompt: "Use the deepstream-dev skill without MCP. Rely on the local `SKILL.md`, bundled references or scripts, and manual verification. Show the exact commands, evidence, and final checks you used before concluding."
...
- [development-workflow](../development-workflow/SKILL.md): Use it when the DeepStream task also needs a scoped implementation and verification plan.
- [devops-tooling](../devops-tooling/SKILL.md): Use it when the pipeline work also needs container, deployment, or CI handling.
- [cloud-design-patterns](../cloud-design-patterns/SKILL.md): Use it when the video stack also needs broader streaming or distributed-system architecture tradeoffs.

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/deploy-to-vercel` and restart Codex after major changes.
...
- Fallback prompt: "Use the Deploy to Vercel skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `deploy-to-vercel` outside its documented task boundary.
...
Before claiming the `deploy-to-vercel` workflow succeeded:

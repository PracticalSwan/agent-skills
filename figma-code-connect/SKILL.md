...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/figma-code-connect` and restart Codex after major changes.
...
- Fallback prompt: "Use the Code Connect skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `figma-code-connect` outside its documented task boundary.
...
Before claiming the `figma-code-connect` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/figma-generate-design` and restart Codex after major changes.
...
- Fallback prompt: "Use the Build / Update Screens and Views from Design System skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `figma-generate-design` outside its documented task boundary.
...
Before claiming the `figma-generate-design` workflow succeeded:

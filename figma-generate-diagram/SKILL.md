...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/figma-generate-diagram` and restart Codex after major changes.
...
- Fallback prompt: "Use the generate-diagram skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `figma-generate-diagram` outside its documented task boundary.
...
Before claiming the `figma-generate-diagram` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/figma-generate-library` and restart Codex after major changes.
...
- Fallback prompt: "Use the Design System Builder — Figma MCP Skill skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `figma-generate-library` outside its documented task boundary.
...
Before claiming the `figma-generate-library` workflow succeeded:

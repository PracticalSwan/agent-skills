...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/figma-implement-motion` and restart Codex after major changes.
...
- Fallback prompt: "Use the Implement Motion skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `figma-implement-motion` outside its documented task boundary.
...
Before claiming the `figma-implement-motion` workflow succeeded:

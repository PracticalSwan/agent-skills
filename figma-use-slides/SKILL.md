...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/figma-use-slides` and restart Codex after major changes.
...
- Fallback prompt: "Use the use_figma — Figma Plugin API Skill for Slides skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `figma-use-slides` outside its documented task boundary.
...
Before claiming the `figma-use-slides` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/file-edit-in-place` and restart Codex after major changes.
...
- Fallback prompt: "Use the File Edit In Place skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `file-edit-in-place` outside its documented task boundary.
...
Before claiming the `file-edit-in-place` workflow succeeded:

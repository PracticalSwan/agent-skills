...
agent = client.agents.create(
    id="code-reviewer",
    base_agent="available-managed-agent-id",
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/gemini-interactions-api` and restart Codex after major changes.
...
- Fallback prompt: "Use the Gemini Interactions API Skill skill without MCP. Follow the official ai.google.dev documentation and current google-genai SDK fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `gemini-interactions-api` outside its documented task boundary.
...
Before claiming the `gemini-interactions-api` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/gemini-live-api-dev` and restart Codex after major changes.
...
- Fallback prompt: "Use the Gemini Live API Development Skill skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `gemini-live-api-dev` outside its documented task boundary.
...
Before claiming the `gemini-live-api-dev` workflow succeeded:

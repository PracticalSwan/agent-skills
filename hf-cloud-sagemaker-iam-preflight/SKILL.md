...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/hf-cloud-sagemaker-iam-preflight` and restart Codex after major changes.
...
- Fallback prompt: "Use the SageMaker IAM Preflight skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `hf-cloud-sagemaker-iam-preflight` outside its documented task boundary.
...
Before claiming the `hf-cloud-sagemaker-iam-preflight` workflow succeeded:

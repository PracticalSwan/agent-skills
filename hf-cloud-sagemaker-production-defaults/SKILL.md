...
tags: [hugging-face, hf, cloud, sagemaker, production, defaults]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/hf-cloud-sagemaker-production-defaults` and restart Codex after major changes.
...
- Fallback prompt: "Use the SageMaker Production Defaults skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `hf-cloud-sagemaker-production-defaults` outside its documented task boundary.
...
Before claiming the `hf-cloud-sagemaker-production-defaults` workflow succeeded:

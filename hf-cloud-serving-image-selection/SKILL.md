...
tags: [hugging-face, hf, cloud, serving, image, selection]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/hf-cloud-serving-image-selection` and restart Codex after major changes.
...
- Fallback prompt: "Use the Serving Image Selection skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `hf-cloud-serving-image-selection` outside its documented task boundary.
...
Before claiming the `hf-cloud-serving-image-selection` workflow succeeded:

...
tags: [hugging-face, huggingface, best]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-best` and restart Codex after major changes.
...
- Fallback prompt: "Use the HuggingFace Best Model Finder skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-best` outside its documented task boundary.
...
Before claiming the `huggingface-best` workflow succeeded:

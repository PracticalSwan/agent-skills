...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-gradio` and restart Codex after major changes.
...
- Fallback prompt: "Use the Gradio skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-gradio` outside its documented task boundary.
...
Before claiming the `huggingface-gradio` workflow succeeded:

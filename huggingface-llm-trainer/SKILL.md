...
tags: [hugging-face, huggingface, llm, trainer]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-llm-trainer` and restart Codex after major changes.
...
- Fallback prompt: "Use the TRL Training on Hugging Face Jobs skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-llm-trainer` outside its documented task boundary.
...
Before claiming the `huggingface-llm-trainer` workflow succeeded:

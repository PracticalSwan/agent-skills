...
tags: [hugging-face, huggingface, lora, space, builder]
...
- If no cached token, or the cached token can't read this private repo, stop at a
  safe authentication handoff. Ask the user to configure an approved secret
  manager or complete `hf auth login` locally; never request or accept a token
  value in chat, commands, or logs.
...
- **401/403 on `upload_file`**: token doesn't have write scope. Fix: ask the
  user to refresh the approved secret or local login with write scope; never
  request the token value in chat.
...
- Waiting until the end to establish an approved HF auth session, then discovering the LoRA was private and the model card was unreadable.
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-lora-space-builder` and restart Codex after major changes.
...
- Fallback prompt: "Use the Gradio LoRA Space Builder skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-lora-space-builder` outside its documented task boundary.
...
Before claiming the `huggingface-lora-space-builder` workflow succeeded:

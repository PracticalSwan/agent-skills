...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-paper-publisher` and restart Codex after major changes.
...
- Fallback prompt: "Use the Overview skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-paper-publisher` outside its documented task boundary.
...
Before claiming the `huggingface-paper-publisher` workflow succeeded:

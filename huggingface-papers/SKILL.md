...
tags: [hugging-face, huggingface, papers]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-papers` and restart Codex after major changes.
...
- Fallback prompt: "Use the Hugging Face Paper Pages skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-papers` outside its documented task boundary.
...
Before claiming the `huggingface-papers` workflow succeeded:

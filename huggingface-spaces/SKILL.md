...
1. Check whether the `hf` CLI is installed (`Get-Command hf` on Windows or
   `command -v hf` on POSIX). If it is absent, use the official Python SDK or
   browser workflow; do not install a runtime implicitly.
2. Check the user is logged in: `hf auth whoami`. If not, ask the user to run
   `hf auth login` locally and complete its browser/OAuth flow, or configure an
   approved secret manager. Never request or accept a token value in chat or
   pass a literal token in a command argument.
...
- `--secrets KEY=val` becomes an environment variable inside the Space and is **not** visible to visitors. Use the CLI's approved secret-store or environment integration for API keys and gated-repo tokens; never put a literal token in chat, source, or command history. Can also be set later through the Space Settings UI or an approved `hf spaces secrets set` flow.
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-spaces` and restart Codex after major changes.
...
- Fallback prompt: "Use the Hugging Face Spaces skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-spaces` outside its documented task boundary.
...
Before claiming the `huggingface-spaces` workflow succeeded:

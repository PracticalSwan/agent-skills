...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-tool-builder` and restart Codex after major changes.
...
- Fallback prompt: "Use the Hugging Face API Tool Builder skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-tool-builder` outside its documented task boundary.
...
Before claiming the `huggingface-tool-builder` workflow succeeded:

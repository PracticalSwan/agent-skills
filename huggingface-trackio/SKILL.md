...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-trackio` and restart Codex after major changes.
...
- Fallback prompt: "Use the Trackio - Experiment Tracking for ML Training skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-trackio` outside its documented task boundary.
...
Before claiming the `huggingface-trackio` workflow succeeded:

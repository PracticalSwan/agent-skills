...
tags: [hugging-face, huggingface, vision, trainer]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-vision-trainer` and restart Codex after major changes.
...
- Fallback prompt: "Use the Vision Model Training on Hugging Face Jobs skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-vision-trainer` outside its documented task boundary.
...
Before claiming the `huggingface-vision-trainer` workflow succeeded:

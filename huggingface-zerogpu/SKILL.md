...
tags: [hugging-face, huggingface, zerogpu]
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/huggingface-zerogpu` and restart Codex after major changes.
...
- Fallback prompt: "Use the Hugging Face ZeroGPU skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `huggingface-zerogpu` outside its documented task boundary.
...
Before claiming the `huggingface-zerogpu` workflow succeeded:

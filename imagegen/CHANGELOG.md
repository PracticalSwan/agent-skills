...
All notable changes to the `imagegen` skill are documented here.
...
- Added capability-based host routing so Codex keeps its built-in image path
  while Claude Code and Copilot require an actually exposed image tool or the

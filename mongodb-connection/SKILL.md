...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/mongodb-connection` and restart Codex after major changes.
...
- Fallback prompt: "Use the MongoDB Connection Optimizer skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `mongodb-connection` outside its documented task boundary.
...
Before claiming the `mongodb-connection` workflow succeeded:

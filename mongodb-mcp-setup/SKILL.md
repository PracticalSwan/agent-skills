...
- **If no `CODEX_*` variables are present** → the user is running a **shell-based client** (Claude, Cursor, Copilot CLI, or another compatible client). Credentials are configured via shell profile environment variables.
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/mongodb-mcp-setup` and restart Codex after major changes.
...
- Fallback prompt: "Use the MongoDB MCP Server Setup skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `mongodb-mcp-setup` outside its documented task boundary.
...
Before claiming the `mongodb-mcp-setup` workflow succeeded:

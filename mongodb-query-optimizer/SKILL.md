...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/mongodb-query-optimizer` and restart Codex after major changes.
...
- Fallback prompt: "Use the MongoDB Query Optimizer skill without MCP. Follow the documented local or manual fallback, show the selected tool surface, and report the verification evidence."
...
- Activating `mongodb-query-optimizer` outside its documented task boundary.
...
Before claiming the `mongodb-query-optimizer` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/netlify-deploy` and restart Codex after major changes.
...
- Fallback prompt: "Use the Netlify deploy skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `netlify-deploy` outside its documented task boundary.
...
Before claiming the `netlify-deploy` workflow succeeded:

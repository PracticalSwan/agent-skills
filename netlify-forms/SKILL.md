...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/netlify-forms` and restart Codex after major changes.
...
- Fallback prompt: "Use the Netlify Forms skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `netlify-forms` outside its documented task boundary.
...
Before claiming the `netlify-forms` workflow succeeded:

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/netlify-functions` and restart Codex after major changes.
...
- Fallback prompt: "Use the Netlify Functions skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `netlify-functions` outside its documented task boundary.
...
Before claiming the `netlify-functions` workflow succeeded:

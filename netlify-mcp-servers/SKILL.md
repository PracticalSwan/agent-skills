...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/netlify-mcp-servers` and restart Codex after major changes.
...
- Fallback prompt: "Use the Netlify MCP Servers skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `netlify-mcp-servers` outside its documented task boundary.
...
Before claiming the `netlify-mcp-servers` workflow succeeded:
...
- [netlify-deploy](../netlify-deploy/SKILL.md): Use it when the task also needs its adjacent workflow.
- [netlify-config](../netlify-config/SKILL.md): Use it when the task also needs its adjacent workflow.

...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/react-view-transitions` and restart Codex after major changes.
...
- Fallback prompt: "Use the React View Transitions skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `react-view-transitions` outside its documented task boundary.
...
Before claiming the `react-view-transitions` workflow succeeded:

...
All notable changes to the `remembering-conversations` skill are documented here.
...
- Hardened troubleshooting and migration guidance so API keys are never printed and legacy data is removed only after exact-path verification and explicit approval.

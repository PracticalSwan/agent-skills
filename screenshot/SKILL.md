...
- Fallback prompt: "Use the Screenshot Capture skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `screenshot` outside its documented task boundary.
...
Before claiming the `screenshot` workflow succeeded:

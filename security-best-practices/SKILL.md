...
- Fallback prompt: "Use the Security Best Practices skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `security-best-practices` outside its documented task boundary.
...
Before claiming the `security-best-practices` workflow succeeded:

...
- Fallback prompt: "Use the Security Ownership Map skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `security-ownership-map` outside its documented task boundary.
...
Before claiming the `security-ownership-map` workflow succeeded:

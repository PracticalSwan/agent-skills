...
1. Identify platform, page type, intent, sections, visual style, components, and constraints already present.
2. Read `.stitch/DESIGN.md` when present and include only relevant rules.
3. Replace vague terms with concrete UI patterns, component names, and layout structure.
4. For generation prompts, describe layout, content, and behavior without duplicating project-level theme tokens.
5. For edit prompts, name the exact screen region and focused change.
6. Return the enhanced prompt plus assumptions that affect the design.
...
1. Pass/fail: Output includes platform, purpose, page structure, and component terms.
2. Pass/fail: It preserves user intent instead of over-designing a different product.
3. Pass/fail: Design-system context is included only when useful.
4. Pass/fail: The prompt is ready for Stitch generation or web UI use.
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/stitch-enhance-prompt` and restart Codex after major changes.
...
- Fallback prompt: "Use the Stitch Enhance Prompt skill without Stitch MCP. No MCP is required; use local DESIGN.md files, prompt references, and user context. Show the exact files, commands, manual Stitch UI steps, and verification evidence used before concluding."
...
- [avoid-ai-writing](../avoid-ai-writing/SKILL.md): Use when the task also needs this adjacent Stitch workflow.

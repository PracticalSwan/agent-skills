...
- Added the `Context leakage across tasks` anti-pattern.
- Added prompt-template links, a Prompt Evolution note, and a quantitative comparison row for review coverage.
...
### Fixed
- Renamed the self-check section to explicit phase-gate questions so branch-readiness criteria are easier to audit.
...
- Added a self-verification phase gate so task, review, and branch-readiness checks are explicit before the workflow claims completion.

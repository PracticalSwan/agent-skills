...
2. **Check `.mcp.json` configuration:**
   Verify the project root has a valid `.mcp.json` with the correct server URL. If it is missing, report that configuration gap first; create or modify project configuration only when the user explicitly requests and authorizes that change.
...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/supabase` and restart Codex after major changes.
...
- Activating `supabase` outside its documented task boundary.
...
Before claiming the `supabase` workflow succeeded:

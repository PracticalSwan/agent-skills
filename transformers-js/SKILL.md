...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/transformers-js` and restart Codex after major changes.
...
- Fallback prompt: "Use the Transformers.js - Machine Learning for JavaScript skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `transformers-js` outside its documented task boundary.
...
Before claiming the `transformers-js` workflow succeeded:

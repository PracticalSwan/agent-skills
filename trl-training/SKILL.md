...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/trl-training` and restart Codex after major changes.
...
- Fallback prompt: "Use the TRL Training Skill skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `trl-training` outside its documented task boundary.
...
Before claiming the `trl-training` workflow succeeded:

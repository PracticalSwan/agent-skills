...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/vercel-optimize` and restart Codex after major changes.
...
- Fallback prompt: "Use the Vercel Optimize skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `vercel-optimize` outside its documented task boundary.
...
Before claiming the `vercel-optimize` workflow succeeded:

...
The bundled `scripts/analyze.sh` helper is read-only and requires a POSIX
shell plus `jq`. On Windows or another host without those dependencies, use
the documented manual inspection or an equivalent local tool and report that
the helper itself was not run.
...
- Activating `web-quality-audit` outside its documented task boundary.

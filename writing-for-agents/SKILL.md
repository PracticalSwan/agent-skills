...
- Codex: install or sync the folder into
  `$CODEX_HOME/skills/writing-for-agents` and restart Codex after major changes.
...
- Fallback prompt: "Use the Writing For Agents skill without MCP. Rely on its local instructions, bundled resources, standard shell or editor tools, and direct verification. Show the evidence used before concluding."
...
- Activating `writing-for-agents` outside its documented task boundary.
...
Before claiming the `writing-for-agents` workflow succeeded:
